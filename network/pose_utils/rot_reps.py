...
def rot6d_fixed_y_to_mat_batch(d6):
    x_raw = d6[..., 0:3]  # bx3
    y_raw = d6[..., 3:6]  # bx3

    y = F.normalize(y_raw, p=2, dim=-1)  # bx3
    z = torch.cross(y, x_raw, dim=-1)  # bx3
    z = F.normalize(z, p=2, dim=-1)  # bx3
    x = torch.cross(z, y, dim=-1)  # bx3

    # (*,3)x3 --> (*,3,3)
    return torch.stack((x, y, z), dim=-1)  # (b,3,3)

def rot6d_fixed_x_to_mat_batch(d6):
    x_raw = d6[..., 0:3]  # bx3
    y_raw = d6[..., 3:6]  # bx3
...
def rot6d_fixed_z_to_mat_batch(d6):
    y_raw = d6[..., 0:3]  # bx3
    z_raw = d6[..., 3:6]  # bx3

    z = F.normalize(z_raw, p=2, dim=-1)  # bx3
    x = torch.cross(z, y_raw, dim=-1)  # bx3
    x = F.normalize(x, p=2, dim=-1)  # bx3
    y = torch.cross(x, z, dim=-1)  # bx3
