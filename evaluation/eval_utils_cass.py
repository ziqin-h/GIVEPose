...
def compute_RT_degree_cm_symmetry(RT_1, RT_2, class_id, handle_visibility, synset_names):
    ...
    T2 = RT_2[:3, 3]
    #if FLAGS.eval_gt_scale:
    #    T1 = T1 / np.cbrt(np.linalg.det(RT_1[:3, :3])) * np.cbrt(np.linalg.det(RT_2[:3, :3]))
...
def compute_size_overlaps(gt_class_ids, gt_scales,
                        pred_class_ids, pred_scales):
    """Finds similarity between prediction and ground truth size.
    Returns:
    """
    # print('num of gt instances: {}, num of pred instances: {}'.format(len(gt_class_ids), len(gt_class_ids)))
    num_pred = len(pred_class_ids)
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j] = compute_size_similar(pred_scales[i], gt_scales[j])

    return overlaps

def compute_match_from_size(overlaps, pred_class_ids, gt_class_ids, size_thres_list):
    num_size_thres = len(size_thres_list)
    num_pred = len(pred_class_ids)
    num_gt = len(gt_class_ids)
    pred_matches = -1 * np.ones((num_size_thres, num_pred))
    gt_matches = -1 * np.ones((num_size_thres, num_gt))
    if num_pred == 0 or num_gt == 0:
        return gt_matches, pred_matches

    assert num_pred == overlaps.shape[0]
    assert num_gt == overlaps.shape[1]
    for s, size_thres in enumerate(size_thres_list):
        for i in range(num_pred):
            # Find best matching ground truth box
            ...
            for j in sorted_ixs:
                ...
                if gt_matches[s, j] > -1 or pred_class_ids[i] != gt_class_ids[j]:
                    continue
                # If we reach IoU smaller than the threshold, end the loop
                if overlaps[i, j] > size_thres:
                    continue
...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100],
                          iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False, eval_recon=False, plot_figure=False, eval_size=False, size_thres_list=[1, 2, 5, 10, 15]):
    ...
    size_pred_matches_all = [
        np.zeros((num_size_thres, 0)) for _ in range(num_classes)]
    size_gt_matches_all = [
        np.zeros((num_size_thres, 0)) for _ in range(num_classes)]
    size_pred_scores_all = [
        np.zeros((num_size_thres, 0)) for _ in range(num_classes)]
    size_aps = np.zeros((num_classes + 1, num_size_thres))
    ...
    for i, result in enumerate(progress):
        ...
        for cls_id in range(1, num_classes):
            ...
            if synset_names[cls_id] != 'mug' and True:
            ...
            if use_matches_for_pose:
                ...
                cls_pred_scales = cls_pred_scales[iou_thres_pred_match > -1] if len(
                    iou_thres_pred_match) > 0 else np.zeros(0)
            ...
            cls_pred_scores_tile_size = np.tile(
                cls_pred_scores, (num_size_thres, 1))

            if eval_size:
                size_overlaps = compute_size_overlaps(cls_gt_class_ids, cls_gt_scales,
                                                  cls_pred_class_ids, cls_pred_scales)
                size_cls_gt_match, size_cls_pred_match = compute_match_from_size(size_overlaps, cls_pred_class_ids, cls_gt_class_ids, size_thres_list)
                size_pred_matches_all[cls_id] = np.concatenate(
                (size_pred_matches_all[cls_id], size_cls_pred_match), axis=-1)
                size_gt_matches_all[cls_id] = np.concatenate(
                    (size_gt_matches_all[cls_id], size_cls_gt_match), axis=-1)

                size_pred_scores_all[cls_id] = np.concatenate(
                    (size_pred_scores_all[cls_id], cls_pred_scores_tile_size), axis=-1)

    if eval_size:
        for j, _ in enumerate(size_thres_list):
            for cls_id in range(1, num_classes):
                cls_size_pred_matches_all = size_pred_matches_all[cls_id][j, :]
                cls_size_gt_matches_all = size_gt_matches_all[cls_id][j, :]
                cls_size_pred_scores_all = size_pred_scores_all[cls_id][j, :]

                size_aps[cls_id, j] = compute_ap_from_matches_scores(cls_size_pred_matches_all,
                                                                     cls_size_pred_scores_all,
                                                                     cls_size_gt_matches_all,
                                                                     )

            size_aps[-1, j] = np.mean(size_aps[1:-1, j])
        return size_aps
    ...
    if plot_figure:
        ...
        ax_iou.grid(True, which='both', linestyle='--', linewidth=0.5)
        plt.ylabel('AP')
        plt.ylim((0, 1))
        ...
        aps_dict['iou'] = iou_3d_aps.tolist()
    ...
    if plot_figure:
        ...
        ax_trans.grid(True, which='both', linestyle='--', linewidth=0.5)
        plt.ylim((0, 1))
        ...
        ax_rot.grid(True, which='both', linestyle='--', linewidth=0.5)
        plt.ylim((0, 1))
        ...
        aps_dict['pose'] = pose_aps.tolist()
        save_path = os.path.join(log_dir, 'aps.json')
        json.dump(aps_dict, open(save_path, 'w'))
...
def compute_size_similar(pred_scale, gt_scale):
    '''
    :param pred_scales: pred norm size [3,]
    :param gt_scales: gt norm size [3,]
    :return: the angle between the two vectors
    '''
    dot_product = np.dot(pred_scale, gt_scale)
    norm_pred_scale = np.linalg.norm(pred_scale)
    norm_gt_scale = np.linalg.norm(gt_scale)

    # 计算夹角的余弦值
    cos_angle = dot_product / (norm_pred_scale * norm_gt_scale)

    # 使用 arccos 函数计算夹角的弧度值
    angle_radians = np.arccos(np.clip(cos_angle, -1.0, 1.0))

    # 将弧度值转换为角度值
    angle_degrees = np.degrees(angle_radians)
    assert 0 <= angle_degrees <= 90, f"{pred_scale, gt_scale}, {angle_degrees}"
